...
const STATUS_OPTIONS: {
  ...
  {
    value: "qatari_self",
    en: "Qatari (Self-sponsored)",
    ar: "قطري (ممول ذاتيًا)",
    descEn: "100% tuition waiver (UG & Foundation)",
    descAr: "إعفاء كامل من الرسوم (بكالوريوس والتأسيسي)",
  },
  {
    value: "qatari_sponsored",
    en: "Qatari (Sponsored)",
    ar: "قطري (ممول من جهة خارجية)",
    descEn: "100% tuition waiver (UG & Foundation)",
    descAr: "إعفاء كامل من الرسوم (بكالوريوس والتأسيسي)",
  },
  {
    value: "qatari_mother_child",
  ...
  {
    value: "udst_employee_child",
    en: "Child of UDST Employee",
    ar: "ابن/ابنة موظف UDST",
    descEn: "50% tuition waiver (UG & Foundation)",
    descAr: "إعفاء 50% من الرسوم (بكالوريوس والتأسيسي)",
  },
  {
    value: "qatar_resident",
    en: "Qatar Resident",
    ar: "مقيم في قطر",
    descEn: "Resident tuition rates",
    descAr: "رسوم المقيمين",
  },
  {
    value: "international",
    en: "International",
    ar: "طالب دولي",
    descEn: "International tuition rates",
    descAr: "رسوم الطلاب الدوليين",
  },
];
...
export default function FeesManager() {
  ...
  const [enrollmentType, setEnrollmentType] = useLocalStorage(
    "fees-enrollmentType",
    "fullTime",
  );
  const [admissionDate, setAdmissionDate] = useLocalStorage(
    "fees-admissionDate",
    "afterFall2020",
  );
  const [sponsorshipType, setSponsorshipType] = useLocalStorage(
    "fees-sponsorshipType",
    "nonSponsored",
  );
  const [studentStatus, setStudentStatus] = useLocalStorage<StudentStatus>(
    "fees-studentStatus",
    "qatar_resident",
  );
  const [courses, setCourses] = useLocalStorage("fees-courses", [] as Course[]);
  const [numCourses, setNumCourses] = useLocalStorage<number>(
    "fees-numCourses",
    1,
  );
  const [includeAdditional, setIncludeAdditional] = useLocalStorage(
    "fees-includeAdditionalFees",
    false,
  );
  const [additionalAmt, setAdditionalAmt] = useLocalStorage(
    "fees-additionalFeesAmount",
    0,
  );
  ...
  const fullExemptStatuses: StudentStatus[] = [
    "qatari_self",
    "qatari_sponsored",
    "qatari_mother_child",
  ];
  const hasTuitionExemption = () =>
    [...fullExemptStatuses, "udst_employee_child" as StudentStatus].includes(
      studentStatus,
    ) && exemptPrograms.includes(programType);
  const hasFullExemption = () =>
    fullExemptStatuses.includes(studentStatus) &&
    exemptPrograms.includes(programType);
    ...
    setCourses((prev) => {
      ...
      for (let i = 0; i < numCourses; i++) {
        out.push(
          prev[i] || {
            id: `course-${i}`,
            name: isRTL ? names[i] || `المقرر ${i + 1}` : `Course ${i + 1}`,
            credits: 3,
            isRepeated: false,
          },
        );
  ...
  useEffect(() => {
    if (!showRepeated())
      setCourses((p) => p.map((c) => ({ ...c, isRepeated: false })));
  }, [studentStatus]);
  useEffect(() => {
    setMajor("");
    if (!showRepeated())
  ...
  useEffect(() => {
    ...
    const isPartialExempt =
      studentStatus === "udst_employee_child" && hasTuitionExemption();
    ...
    setFeeBreakdown({
      tuitionFees: tuition,
      materialsSupplies: materials,
      studentServices: services,
      workTermFees: 0,
      additionalFees: additionalAmt,
      total,
    });
  }, [
    step,
  ...
  function calcTuition(isIntl: boolean) {
    if (programType === "foundation" || programType === "diploma") {
      const s = feeRates.foundation[
        admissionDate as keyof typeof feeRates.foundation
      ] as any;
      if (admissionDate === "afterFall2020") {
        const r = s[enrollmentType];
        return enrollmentType === "fullTime"
          ? isIntl
            ? r.international
            : r.qatarResident
          : courses.length * r.qatarResident;
      }
      const r = s[sponsorshipType];
      return enrollmentType === "fullTime"
        ? r.fullTime
        : courses.length * r.partTime;
    }
    if (programType === "tcp") {
      const r = feeRates.tcp[admissionDate as keyof typeof feeRates.tcp];
      return enrollmentType === "fullTime"
        ? r.fullTime
        : courses.length * r.partTime;
    }
    if (["bachelor", "postDiploma", "preMaster"].includes(programType)) {
      const mr = feeRates.bachelor[major as keyof typeof feeRates.bachelor];
      if (!mr) return 0;
      return (
        courses.reduce((s, c) => s + c.credits, 0) *
        (isIntl ? mr.international : mr.qatarResident)
      );
    }
    if (programType === "graduate") {
      const mr = feeRates.graduate[major as keyof typeof feeRates.graduate];
      if (!mr) return 0;
      return (
        courses.reduce((s, c) => s + c.credits, 0) *
        (isIntl ? mr.international : mr.qatarResident)
    ...
    if (programType === "postgraduate") {
      const mr =
        feeRates.postgraduate[major as keyof typeof feeRates.postgraduate];
      if (!mr) return 0;
      return (
        courses.reduce((s, c) => s + c.credits, 0) *
        (isIntl ? mr.international : mr.qatarResident)
  ...
  function calcRepeatedFees(rep: Course[]) {
    ...
    if (programType === "foundation" || programType === "diploma") {
      ...
      if (admissionDate === "afterFall2020")
        return rep.length * s.partTime.qatarResident;
      return rep.length * s[sponsorshipType].partTime;
    }
    if (programType === "tcp")
      return (
        rep.length *
        feeRates.tcp[admissionDate as keyof typeof feeRates.tcp].partTime
      );
    if (["bachelor", "postDiploma", "preMaster"].includes(programType)) {
      const mr = feeRates.bachelor[major as keyof typeof feeRates.bachelor];
      if (!mr) return 0;
      return (
        rep.reduce((s, c) => s + c.credits, 0) *
        (isIntl ? mr.international : mr.qatarResident)
      );
    }
    if (programType === "graduate") {
  ...
  function calcMaterials() {
    if (["foundation", "diploma", "tcp"].includes(programType))
  ...
  const selClass =
    "w-full bg-zinc-800/60 border border-zinc-600/40 rounded-lg px-3 py-2.5 text-sm text-white appearance-none cursor-pointer focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-colors";
          ...
          <div className="flex items-center justify-center mb-8 sm:mb-10">
            {STEPS.map((s, i) => (
              <div key={s.n} className="flex items-center">
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className={`w-6 sm:w-10 h-px mx-1 ${step > s.n ? "bg-blue-500/30" : "bg-zinc-700/40"}`}
                  />
                )}
              </div>
            ))}
            ...
            <div className="space-y-6 sm:space-y-8">
              <Card title={t("step1")} className={cardClass}>
                <p className="text-xs text-zinc-500 mb-5">
                  {isRTL
                    ? "اختر نوع البرنامج والتخصص ونوع التسجيل."
                    : "Select your program, major, and enrollment type."}
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="foundation">
                        {isRTL ? "برنامج التأسيسي" : "Foundation"}
                      </option>
                      <option value="diploma">
                        {isRTL ? "دبلوم" : "Diploma"}
                      </option>
                      <option value="tcp">
                        {isRTL ? "شهادة التقني" : "TCP"}
                      </option>
                      <option value="bachelor">
                        {isRTL ? "بكالوريوس" : "Bachelor's"}
                      </option>
                      <option value="postDiploma">
                        {isRTL ? "ما بعد الدبلوم" : "Post-Diploma"}
                      </option>
                      <option value="preMaster">
                        {isRTL ? "ما قبل الماجستير" : "Pre-Master"}
                      </option>
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="engineering">
                          {isRTL ? "الهندسة والتكنولوجيا" : "Engineering"}
                        </option>
                        <option value="generalEducation">
                          {isRTL ? "التعليم العام" : "General Education"}
                        </option>
                        <option value="healthSciences">
                          {isRTL ? "العلوم الصحية" : "Health Sciences"}
                        </option>
                      </select>
                    </div>
                  )}
                  {programType === "postgraduate" && (
                    <div>
                      ...
                      >
                        <option value="">{isRTL ? "اختر" : "Select"}</option>
                        <option value="neonatalCare">
                          {isRTL ? "العناية بحديثي الولادة" : "Neonatal Care"}
                        </option>
                        <option value="stemEducation">
                          {isRTL
                  ...
                  {["foundation", "diploma", "tcp"].includes(programType) && (
                    <div>
                      <label className="block text-[11px] font-medium text-zinc-500 mb-1.5">
                        {isRTL ? "تاريخ القبول" : "Admission Date"}
                      </label>
                      <select
                        value={admissionDate}
                        ...
                        className={selClass}
                      >
                        <option value="afterFall2020">
                          {isRTL ? "خريف 2020 أو بعدها" : "Fall 2020 or later"}
                        </option>
                        <option value="beforeFall2020">
                          {isRTL ? "قبل خريف 2020" : "Before Fall 2020"}
                        </option>
                      </select>
                    </div>
                  )}
                  ...
                  {/* sponsorship */}
                  {["foundation", "diploma"].includes(programType) &&
                    admissionDate === "beforeFall2020" && (
                      <div>
                        <label className="block text-[11px] font-medium text-zinc-500 mb-1.5">
                          {isRTL ? "نوع الرعاية" : "Sponsorship"}
                        </label>
                        <select
                          ...
                          className={selClass}
                        >
                          <option value="nonSponsored">
                            {isRTL ? "غير مدعوم" : "Non-sponsored"}
                          </option>
                          <option value="sponsored">
                            {isRTL ? "مدعوم" : "Sponsored"}
                          </option>
                        </select>
                      </div>
                    )}
              ...
              <Card title={t("status")} className={cardClass}>
                <p className="text-xs text-zinc-500 mb-4">
                  {isRTL
                    ? "حالتك تحدد الرسوم والإعفاءات."
                    : "Your status determines tuition rates and waivers."}
                </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p
                            className={`text-sm font-medium ${sel ? "text-white" : "text-zinc-300"}`}
                          >
                            {isRTL ? opt.ar : opt.en}
                          </p>
          ...
          {step === 2 && (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 sm:gap-6">
              <Card title={t("step2")} className={`${cardClass} lg:col-span-3`}>
                <p className="text-xs text-zinc-500 mb-5">
                  {isRTL
                    ? "أضف المقررات والساعات المعتمدة."
                ...
                <div className="space-y-3">
                  {courses.map((c, i) => (
                    <div
                      key={c.id}
                      className="rounded-xl border border-zinc-600/25 bg-zinc-700/15 px-4 py-3.5 sm:px-5"
                    >
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {showRepeated() && (
                          <button
                            type="button"
                            onClick={() =>
                              updateCourse(i, "isRepeated", !c.isRepeated)
                            }
                ...
                <div className="mt-5 pt-4 border-t border-zinc-700/30">
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded border flex items-center justify-center ${includeAdditional ? "bg-blue-500 border-blue-500" : "border-zinc-600 bg-transparent"}`}
                    >
                      {includeAdditional && (
                        <svg
                          viewBox="0 0 20 20"
                  ...
                  {includeAdditional && (
                    <div className="mt-3">
                      <input
                        ...
                        value={additionalAmt}
                        onChange={(e) =>
                          setAdditionalAmt(parseInt(e.target.value) || 0)
                        }
                        placeholder="0"
                        className={`${selClass} w-full sm:w-48`}
                      />
                      <p className="text-[10px] text-zinc-500 mt-1">
                        {isRTL
                          ? "سكن، طباعة، إلخ (بالريال)"
                          : "Housing, printing, etc. (QAR)"}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <Card className={`${cardClass} lg:col-span-2`}>
                ...
                <div className="rounded-xl border border-zinc-600/25 bg-zinc-800/20 overflow-hidden mb-5">
                  {courses.map((c, i) => (
                    <div
                      key={c.id}
                      className={`flex items-center justify-between px-3.5 py-2.5 ${i > 0 ? "border-t border-zinc-700/25" : ""}`}
                    >
                ...
                {hasFullExemption() && (
                  <div className="rounded-lg border border-emerald-500/15 bg-emerald-500/[0.04] px-3 py-2.5 mb-5">
                    <p className="text-xs text-emerald-400">
                      {isRTL
                        ? "معفى من الرسوم. المقررات المعادة فقط تُحسب."
                        : "Tuition exempt. Only repeated courses are charged."}
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="flex-1 px-4 py-2.5 rounded-xl border border-zinc-600/40 text-zinc-400 text-sm font-medium hover:text-white hover:bg-zinc-700/30 transition-colors"
                  >
                    {t("prev")}
                  </button>
                  <button
              ...
              <Card className={`${cardClass} lg:col-span-3`}>
                ...
                <div className="rounded-xl border border-zinc-600/25 bg-zinc-800/20 overflow-hidden">
                  {[
                    {
                      label: t("tuition"),
                      value: feeBreakdown.tuitionFees,
                      freeCheck:
                        hasFullExemption() && feeBreakdown.tuitionFees === 0,
                    },
                    {
                      label: t("materials"),
                      value: feeBreakdown.materialsSupplies,
                      freeCheck:
                        hasFullExemption() &&
                        feeBreakdown.materialsSupplies === 0,
                    },
                    {
                      label: t("services"),
                      value: feeBreakdown.studentServices,
                      freeCheck:
                        hasFullExemption() &&
                        feeBreakdown.studentServices === 0,
                    },
                    ...(feeBreakdown.additionalFees > 0
                      ? [
                          {
                            label: t("additional"),
                            value: feeBreakdown.additionalFees,
                            freeCheck: false,
                          },
                        ]
                      : []),
                    ...
                    <div
                      key={i}
                      className={`flex items-center justify-between px-4 py-3 sm:px-5 ${i > 0 ? "border-t border-zinc-700/25" : ""}`}
                    >
                      <span className="text-xs sm:text-sm text-zinc-400">
                        {row.label}
                      </span>
                      <span
                        className={`text-xs sm:text-sm font-semibold tabular-nums ${row.freeCheck ? "text-emerald-400" : "text-white"}`}
                      >
                        {row.freeCheck
                          ? t("free")
                          : `${row.value.toLocaleString()} ${t("qar")}`}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                {hasTuitionExemption() && (
                  <p className="text-xs text-zinc-400 mt-4 pt-3 border-t border-zinc-700/30">
                    {hasFullExemption()
                      ? isRTL
                        ? "إعفاء كامل. تدفع فقط للمقررات المعادة."
                        : "Full exemption. Only repeated courses are charged."
                      : isRTL
                ...
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  className="w-full mt-6 px-4 py-2.5 rounded-xl border border-zinc-600/40 text-zinc-400 text-sm font-medium hover:text-white hover:bg-zinc-700/30 transition-colors"
                >
                  {t("prev")}
                </button>
              </Card>
                ...
                <div className="rounded-xl border border-zinc-600/25 bg-zinc-800/20 overflow-hidden">
                  {[
                              ...
                              : programType === "tcp"
                                ...
                                : programType === "diploma"
                                  ? isRTL
                                    ? "دبلوم"
                                    : "Diploma"
                                  : programType === "postDiploma"
                                    ? isRTL
                                      ? "ما بعد الدبلوم"
                                      : "Post-Diploma"
                                    : programType === "preMaster"
                                      ? isRTL
                                        ? "ما قبل الماجستير"
                                        : "Pre-Master"
                                      : isRTL
                                ...
                                : major === "computing"
                                  ...
                                  : major === "engineering"
                                    ? isRTL
                                      ? "الهندسة"
                                      : "Engineering"
                                    : major === "generalEducation"
                                      ? isRTL
                                        ? "التعليم العام"
                                        : "General Ed"
                                      : major === "healthSciences"
                                        ? isRTL
                                          ? "العلوم الصحية"
                                          : "Health Sci"
                                        : major === "neonatalCare"
                    ...
                    },
                    {
                      label: t("status"),
                      value:
                        STATUS_OPTIONS.find((o) => o.value === studentStatus)?.[
                          isRTL ? "ar" : "en"
                        ] || "",
                    },
                    {
                      label: isRTL ? "المقررات" : "Courses",
                      value: `${courses.length}`,
                    },
                    {
                      label: isRTL ? "الساعات" : "Credits",
                      value: `${totalCredits}`,
                    },
                    ...(showRepeated() && courses.some((c) => c.isRepeated)
                      ? [
                          {
                            label: isRTL ? "معادة" : "Repeated",
                            value: `${courses.filter((c) => c.isRepeated).length}`,
                          },
                        ]
                      : []),
                  ].map((row, i) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-white text-end truncate max-w-[55%]">
                        {row.value}
                      </span>
                    </div>
                  ))}
